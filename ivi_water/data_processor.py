...
class DataProcessor:
    ...
    def _clean_water_data(
        ...
        try:
            ...
            df_clean.sort_values(
                ["location_id", "year", "season"], inplace=True, ignore_index=True
            )
    ...
    def _clean_nrm_data(self, df: pd.DataFrame, inplace: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            # Optimization: Use ignore_index=True to avoid creating and immediately dropping an index object
            df_clean.sort_values(
                ["location_id", "year"], inplace=True, ignore_index=True
            )
    ...
    def aggregate_by_intervention(
        ...
        try:
            ...
            # Optimization: Calculate location_id_nunique directly using groupby().nunique()
            # This avoids O(G * N) boolean masking evaluations in Python and utilizes pandas'
            # highly optimized C grouping engine, which is orders of magnitude faster even
            # when location_id is not categorical.
            agg_stats["location_id_nunique"] = grouped["location_id"].nunique()
