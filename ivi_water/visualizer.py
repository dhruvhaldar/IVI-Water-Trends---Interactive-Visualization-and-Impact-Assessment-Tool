...
class WaterTrendsVisualizer:
    ...
    def __init__(
        ...
        if not isinstance(height, int) or height < 200 or height > 2000:
            raise ValueError(
                f"Chart height must be between 200-2000 pixels, got {height}"
            )

        if not isinstance(width, int) or width < 400 or width > 4000:
            raise ValueError(
                f"Chart width must be between 400-4000 pixels, got {width}"
            )
    ...
    def _create_empty_state_figure(
        ...
        # Ensure WCAG AA contrast requirements are met across themes
        text_color = "#e0e0e0" if self.theme and "dark" in self.theme else "#555555"
    ...
    def create_seasonal_stacked_area_chart(
        ...
        if location_id is not None:
            if "location_id" not in df.columns:
                raise ValueError(
                    "DataFrame must contain 'location_id' column when location_id is specified"
                )
        ...
        try:
            ...
            if location_id:
                ...
                self.logger.debug(
                    f"Aggregated data across {df['location_id'].nunique()} locations"
                )
            ...
            if (df_filtered["water_area_ha"] < 0).any():
                self.logger.warning(
                    "Found negative water area values, these will be displayed as-is"
                )
            ...
            for season in available_seasons:
                ...
                color = SEASON_COLORS.get(
                    season, "#7f7f7f"
                )  # Default gray if color not defined
            ...
            fig.update_layout(
                ...
                xaxis_title=dict(
                    text="Year", font=dict(size=12, family="Arial, sans-serif")
                ),
                yaxis_title=dict(
            ...
            self.logger.error(
                f"Error creating seasonal stacked area chart: {e}", exc_info=True
            )
    ...
    def create_intervention_impact_scatter(
        ...
        df["intervention_status"] = df[intervention_col].map(
            {0: "Without Intervention", 1: "With Intervention"}
        )
    ...
    def create_multi_location_dashboard(
        self, df: pd.DataFrame, location_ids: List[str], save_path: Optional[str] = None
    ) -> go.Figure:
        """
...
def quick_seasonal_plot(
    df: pd.DataFrame, location_id: Optional[str] = None
) -> go.Figure:
